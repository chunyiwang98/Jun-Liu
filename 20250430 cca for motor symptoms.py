...
scales = df.iloc[:, 6:20].values  # G 到 T
...
print(f"best n_components: {n_components}")
...
for i in range(n_components):
    print(f"成分 {i + 1}: 相关系数 = {observed_corrs[i]:.4f}, p 值 = {p_values[i]:.6f}")
...
clinical_scales = df.columns[6:20].tolist()  
...
eeg_features_list = df.columns[26:45].tolist()  # EEG 特征名称
...
for feature in eeg_features_list:
    if 'PSD' in feature:
        ...
        eeg_colors.append('gray') 



plt.figure(figsize=(6, 4))
plt.bar(range(loadings_scales.shape[0]), loadings_scales[:, 0], color='#1f77b4') 
plt.title('Motor Symptoms', fontsize=13, fontfamily='Calibri')
plt.xlabel('', fontsize=10, fontfamily='Calibri')
plt.ylabel('CCA Loadings', fontsize=13, fontfamily='Calibri')
plt.xticks(ticks=np.arange(len(clinical_scales)), labels=clinical_scales, rotation=45, ha='right', fontsize=10, fontfamily='Calibri')
plt.tight_layout()
plt.show()


plt.figure(figsize=(10, 4))
plt.bar(range(loadings_eeg.shape[0]), loadings_eeg[:, 0], color=eeg_colors)
plt.title('EEG Features', fontsize=13, fontfamily='Calibri')
plt.xlabel('', fontsize=10, fontfamily='Calibri')
plt.ylabel('CCA Loadings', fontsize=13, fontfamily='Calibri')
plt.xticks(ticks=np.arange(len(eeg_features_list)), labels=eeg_features_list, rotation=45, ha='right', fontsize=10, fontfamily='Calibri')
plt.tight_layout()
plt.show()


component_idx = 0  # 成分 3（索引从 0 开始）
...
plt.title("r = 0.370, p = 0.00770", fontsize=10)
plt.xlabel("Canonical Scores of Motor Symptoms", fontsize=10)
